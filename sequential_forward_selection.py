...
def feature_sel(data_):
    ...
    x_sel = list()
    x_rem = [i for i in range(0, 10)]
    prev_min_cros_entrpy = 100.0
    min_cros_entrpy = 1.0
    while (prev_min_cros_entrpy - (0.01 * prev_min_cros_entrpy)) > min_cros_entrpy:
        ...
        for feat in x_rem:
            model_ = MLPClassifier()
            print('Training the model with feature(s): {}'.format([feat] + x_sel))
            model_.fit(train_[:, [feat]+x_sel], train_[:, LABEL_COL])
            # print(model_.classes_)
            print('Calculating entropy')
            crs_entr_li.append(cross_entropy(model_, valid_, [feat]+x_sel))
        print('Entropies are: {}'.format(crs_entr_li))
        print('Minimum entropy is {} and is obtained for feature: {}'
              .format(min(crs_entr_li), x_rem[crs_entr_li.index(min(crs_entr_li))]))
        min_cros_entrpy = min(crs_entr_li)
        x_sel.append(x_rem[crs_entr_li.index(min(crs_entr_li))])
        x_rem.pop(crs_entr_li.index(min(crs_entr_li)))
        print('Selected features in this iteration: {}'.format(x_sel))
        print('Remaining features in this iteration: {}'.format(x_rem))
